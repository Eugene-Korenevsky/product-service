...
var BucketService = class {
  ...
  async moveBucketObject(filePath) {
    const dd = `${filePath.replace(uploadedFolderName, parsedFolderName)}`;
    console.log(dd);
    this.s3Client.send(
...
var handler = async (event) => {
  try {
    ...
    for await (const record of event.Records) {
      const bucketName2 = record.s3.bucket.name;
      const key = record.s3.object.key;
      const stream = await bucketService.getObject(bucketName2, key);
